<template>
  <div>
    ...
    <div class="flexbox">
      <div>Time: {{ data[value].DateTime }}</div>
      <div class="spacer" />
      <div v-show="this.value > 13" class="prediction">Prediction</div>
    </div>
...
export default {
  ...
  mounted() {
    // update data set
    d3.select('#forecastRadarChart *').remove()
    if (this.value - 20 < 0)
    ...
    let opacityScale = d3.scaleLinear()
      .domain([0, 19])
      .range( [0.2, 1] ) // hard coded - number of points displayed at a time
    // -----------------------------------------------------------------------------------------
