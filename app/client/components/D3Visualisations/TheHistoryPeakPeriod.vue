  ...
  mounted() {
    ...
    const mouseoverRott = function (event, d) {
      ...
      svg
        ...
        .attr('y', () => {
          return y(d.CottPeakPeriodPred) - 15
        })
        .text(() => {
          return parseFloat(d.CottPeakPeriodPred).toFixed(2)
        }) // Value of the text
    }
    ...
    svg
      ...
      .attr('d',
        d3.line()
          .x((d) => { return x(parseDateTime(d.DateTime)) })
          .y((d) => { return y(d.CottPeakPeriodPred) })
      )
    ...
    svg
      ...
      .attr('class', 'rottPeakPeriod')
      .attr('cx', (d) => { return x(parseDateTime(d.DateTime)) })
      .attr('cy', (d) => { return y(d.CottPeakPeriodPred) })
      .attr('r', (d) => this.data.length <= 50 ? 1:0)
      .attr('fill', colour.navy)
      .attr('stroke', colour.navy)
      .attr('stroke-width', stroke.pointwidth)
    ...
    if (this.width > 500) {
      ...
      svg.append("text").attr("x", legend.xpos + 18 + 100).attr("y", legend.ypos + 1).text("Predicted Cott Period").style("cursor", "pointer").style("font-size", "15px").attr("alignment-baseline","middle").on("click", toggleRott).on("mouseover", mouseoverLegendRott).on("mouseleave", mouseleaveLegendRott)
