<template>
  <div>
    <div id="TheHistoryForecastPeakPeriod"></div>
  </div>
</template>
...
export default {
  name: 'TheHistoryForecastPeakPeriod',
  props: {
    data: {
      type: Array,
      required: true,
  ...
  mounted() {
    ...
    let svg = d3
      .select('#TheHistoryForecastPeakPeriod')
      .append('svg')
    ...
    // [ Construct Outcome Data - What the actual values are ]
    // ADD LINE PATH
    svg
      .append('path')
      .datum(this.outcome)
    ...
    if (this.width > 500) {
      ...
      svg.append("text").attr("x", legend.xpos + 18 + 100).attr("y", legend.ypos + 1).text("Forecasted Cott Period").style("cursor", "pointer").style("font-size", "15px").attr("alignment-baseline","middle").on("click", toggleRott).on("mouseover", mouseoverLegendRott).on("mouseleave", mouseleaveLegendRott)
