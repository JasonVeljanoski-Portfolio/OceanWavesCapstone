<template>
  <div>
    <div id="TheHistoryForecastWaveHeight"></div>
  </div>
</template>
...
export default {
  name: 'TheHistoryForecastWaveHeight',
  props: {
    data: {
      type: Array,
      required: true,
  ...
  mounted() {
    ...
    let svg = d3
      .select('#TheHistoryForecastWaveHeight')
      .append('svg')
    ...
    if (this.width > 500) {
      ...
      svg.append("text").attr("x", legend.xpos + 18 + 100).attr("y", legend.ypos + 1).text("Forecasted Cott Height").style("cursor", "pointer").style("font-size", "15px").attr("alignment-baseline","middle").on("click", toggleRott).on("mouseover", mouseoverLegendRott).on("mouseleave", mouseleaveLegendRott)
